...
\begin{dfn}
	...
	The singular support $\SSrm(M)$ is the support of $\Ext^*(\onebb_{\End(M)}, \onebb_{\End(M)})$ over $T^* X[2]$.
...
The support (over $T^* X[2]$) of $\Hom(M, N)$ is contained in $\SSrm(M) \cap \SSrm(N)$.
...
Let 
\[
	\Cc^\pre(X) = \Perf(X \times \AA^1)\dMod / \bset{M}{\SSrm(M) \subset \{ \tau = 0 \}}.
\]
...
\]
and let $\mu\supp(M) := \rho(\SSrm(M) \cap \{ \tau \neq 0 \})$.
...
\begin{ex}
	Suppose $i: Z \to X$ is a closed embedding, so $\SSrm(\Perf(Z)) = T^*_Z X$.
	We also have $\Perf(Z) \boxtimes \Perf(0) = \Perf(Z \times 0)$, so $\SSrm(\Perf(Z \times 0)) = T^*_{Z \times 0}(X \times \AA^1)$ and $\mu\supp(\Perf(Z \times 0)) = T^*_Z X$ as desired.
\end{ex}

\begin{ex}
	Let $X = \AA^n$ or $\GG_m^n$.
	Take $Z = \{ y + f(x) = 0 \} \subset X \times \AA^1$.
	Then $\SSrm(\Perf(Z)) = \{ (x, -f(x), c \cdot df, c) \}$ and $\mu\supp(\Perf(Z)) = \Gamma_{df}$.
\end{ex}
